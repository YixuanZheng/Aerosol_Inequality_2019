...
'''
This code generates Fig. S16

Country-level percentage changes in GDP associated with anthropogenic aerosol-induced cooling calculated based on different climatological temperature datasets.
...
for ids,ds in enumerate(datasets):
    ...
    plt.xticks(size=16) 
    plt.yticks(size=16)
    plt.ylabel('GDP changes linked to aerosol-induced cooling (%)',size=16)
    plt.xlabel('Cumulative share of global GDP (%)',size=16)
    
    #plot colorbar
    if ids == 0:
        ax_cb = fig.add_axes([0.74,0.67,0.03,0.08], frame_on=False)
